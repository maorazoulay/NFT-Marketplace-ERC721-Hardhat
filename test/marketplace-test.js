...
describe("NFT", function () {
  ...
  it("Should create a new NFT item on the Marketplace", async function(){
    ...
    itemId = await marketplace.createMarketItem(1, nft.address);
  ...
  it("Should put a new item on sale", async function(){
    ...
    await marketplace.listItemOnSale(0, listingFees, {value: listingFees});
  ...
  it("Should sell the NFT", async function(){
    ...
    const listingFees = ethers.utils.parseUnits('0.025', 'ether');
    await marketplace.connect(buyer).sellMarketItem(0, {value: listingFees})
  ...
  it("Should fail - NFT doesn't exists and can't be sold", async function(){
    ...
    let owner;
    try{
      await marketplace.connect(buyer).sellMarketItem(2, {value: listingFees})
